"""
Background research pipeline — runs in a daemon thread on the main server.
...
def run_full_research(session_id: str):
    """
    Orchestrates the full agentic research pipeline for a session.
    Steps: plan → tool execution → thesis → pitch → audio

    Intended to be called in a background thread via threading.Thread.
    """
    # Ensure this thread gets a fresh DB connection (important for threads).
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.warning("Slide manifest generation failed for %s: %s", session_id, exc)
            ...
            if slides:
                try:
                    ...
                    logger.warning("Pitch HTML build failed for %s: %s", session_id, exc)
            ...
            if html_content:
                try:
                    ...
                    logger.warning("PDF conversion failed for %s: %s", session_id, exc)
            ...
            try:
                ...
                logger.warning("Audio narration generation failed for %s: %s", session_id, exc)
            ...
            logger.exception("Research pipeline failed for session %s: %s", session_id, exc)
        ...
        # Release the DB connection used by this thread so it doesn't linger.
        close_old_connections()
